...
// HandleAddSectionStationAPI dodaje postojeće postaje kao mjerodavne vodomjere dionice
func (h *StationsHandler) HandleAddSectionStationAPI(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	currUser, _ := ctx.Value(contextKeyUser).(*models.User)
	perms, _ := ctx.Value(contextKeyPerms).(*models.UserPermissions)
	...
	if err := h.stationService.AddSectionStations(ctx, perms, code, stationIDs); err != nil {
		if wantsPage(r) {
	...
	if wantsPage(r) {
		redirectWith(w, r, "/sections/"+code, "success", "Vodomjer je dodan na dionicu.")
		return
	}
...
// HandleRemoveSectionStationAPI uklanja vodomjer s dionice, ne iz registra
func (h *StationsHandler) HandleRemoveSectionStationAPI(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	currUser, _ := ctx.Value(contextKeyUser).(*models.User)
	perms, _ := ctx.Value(contextKeyPerms).(*models.UserPermissions)
	...
	if len(stationIDs) != 1 {
		http.Error(w, "Uklanja se jedan vodomjer po zahtjevu", http.StatusBadRequest)
		return
	}

	if err := h.stationService.RemoveSectionStation(ctx, perms, code, stationIDs[0]); err != nil {
		if wantsPage(r) {
			redirectWith(w, r, "/sections/"+code, "error", err.Error())
			return
		}
	...
	if wantsPage(r) {
		redirectWith(w, r, "/sections/"+code, "success", "Vodomjer je uklonjen s dionice; u registru ostaje.")
		return
	}
