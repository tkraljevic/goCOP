...
func scanStation(scanner interface{ Scan(...any) error }) (models.Station, error) {
	...
	if povratni != "" && povratni != "[]" {
		_ = json.Unmarshal([]byte(povratni), &st.ReturnLevels)
	}
...
// returnLevelsJSON pakira povratne vodostaje u stupac; prazno je "[]".
func returnLevelsJSON(st *models.Station) string {
	if len(st.ReturnLevels) == 0 {
		return "[]"
	}
	b, err := json.Marshal(st.ReturnLevels)
	if err != nil {
		return "[]"
	}
	return string(b)
...
// extremesJSON pakira zabilježene ekstreme u stupac; prazno je "[]".
func extremesJSON(st *models.Station) string {
	if len(st.Extremes) == 0 {
		return "[]"
	}
	b, err := json.Marshal(st.Extremes)
	if err != nil {
		return "[]"
	}
	return string(b)
