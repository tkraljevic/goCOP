...
func InitSchema(database *sql.DB) error {
	queries := []string{
		...
		`CREATE INDEX IF NOT EXISTS idx_mts_promet_teren ON mts_promet(journal_id, area_id, section_code, vrsta_id);`,
		...
		`CREATE TABLE IF NOT EXISTS akti (
			...
			kljuc_cvora TEXT NOT NULL DEFAULT '',
		...
		// Izvornik akta: sken akta s vlastoručnim potpisom i žigom; čuva se
		...
		`CREATE TABLE IF NOT EXISTS vodocuvarski_listovi (
			...
			prilike TEXT NOT NULL DEFAULT '',
			naredbe TEXT NOT NULL DEFAULT '',
			opis TEXT NOT NULL DEFAULT '',
			zapazanja TEXT NOT NULL DEFAULT '',
		...
		`CREATE INDEX IF NOT EXISTS idx_vodocuvarski_user ON vodocuvarski_listovi(user_id, datum);`,
		`CREATE INDEX IF NOT EXISTS idx_vodocuvarski_area ON vodocuvarski_listovi(sektor, area_id, datum);`,
		// Zadaci rukovoditelja vodočuvaru; otvoreni stoje na svakom novom listu
		`CREATE TABLE IF NOT EXISTS vodocuvarski_zadaci (
			id TEXT PRIMARY KEY,
			user_id TEXT NOT NULL,
			sektor TEXT NOT NULL DEFAULT '',
			area_id INTEGER NOT NULL DEFAULT 0,
			tekst TEXT NOT NULL,
			zadao_id TEXT NOT NULL DEFAULT '',
			zadao TEXT NOT NULL DEFAULT '',
			zadano_at DATETIME NOT NULL,
			za TEXT NOT NULL DEFAULT '',
			status TEXT NOT NULL DEFAULT 'OTVOREN',
			obavljeno TEXT NOT NULL DEFAULT '',
			obavljeno_at DATETIME,
			list_id TEXT NOT NULL DEFAULT '',
			cvor TEXT NOT NULL DEFAULT '',
			updated_at DATETIME NOT NULL
		);`,
		`CREATE INDEX IF NOT EXISTS idx_vodocuvarski_zadaci ON vodocuvarski_zadaci(user_id, status, zadano_at);`,
		...
		`CREATE TABLE IF NOT EXISTS stations (
			...
			zero_datum_baltic REAL,
			zero_datum_baltic_system TEXT NOT NULL DEFAULT '',
			zero_datum_baltic_source TEXT NOT NULL DEFAULT '',
		...
		`CREATE TABLE IF NOT EXISTS readings (
			...
			temp_c REAL,
			flow_m3s REAL,
			temp_note TEXT NOT NULL DEFAULT '',
			flow_method TEXT NOT NULL DEFAULT '',
...
func migrateSchema(database *sql.DB) error {
	...
	added := []struct {
		...
		{"stations", "zero_datum_baltic", "REAL"},
		{"stations", "zero_datum_baltic_system", "TEXT NOT NULL DEFAULT ''"},
		{"stations", "zero_datum_baltic_source", "TEXT NOT NULL DEFAULT ''"},
		...
		{"vodocuvarski_listovi", "rekonstrukcija", "INTEGER NOT NULL DEFAULT 0"},
		{"vodocuvarski_listovi", "izvor", "TEXT NOT NULL DEFAULT ''"},
		{"vodocuvarski_zadaci", "izvor", "TEXT NOT NULL DEFAULT ''"},
		{"vodocuvarski_zadaci", "obilazak", "TEXT NOT NULL DEFAULT ''"},
		{"vodocuvarski_listovi", "prilozi", "TEXT NOT NULL DEFAULT '[]'"},
