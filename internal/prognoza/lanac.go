...
// Lanac: od čega se koja letva računa, koje su joj rezerve, u čemu se vodi i
// kako se namješta. Stajalo je u naredbi namjesti-prognozu; ovdje je da isti
// postupak radi i aplikacija (Prognoze → Pripremi model) i naredba.
...
// RazaberiRacun čita "letva = ulaz + ulaz".
func RazaberiRacun(zapis string) (Racun, error) {
...
// RastaviLetvu vadi letvu i veličinu iz zapisa "letva" ili "letva:velicina". Druga
// se navodi kad se postavka isprobava bez diranja koda:
// "vrbovka:vodostaj = terezino-polje:protok".
func RastaviLetvu(zapis string) (string, string, error) {
...
func namjestiRacun(arhiva *sql.DB, r Racun, w io.Writer) []Pojas {
	if kod, _, _ := strings.Cut(r.Letva, ":"); RijetkeLetve[strings.TrimSpace(kod)] {
		staro, staroPojas := NajmanjeSati, NajmanjeSatiPojasa
		NajmanjeSati, NajmanjeSatiPojasa = 100, 20
		defer func() { NajmanjeSati, NajmanjeSatiPojasa = staro, staroPojas }()
	}
	letva, vel, err := RastaviLetvu(r.Letva)
	if err != nil {
		fmt.Fprintln(w, err)
		return nil
	}
	izvori := make([]Izvor, 0, len(r.Ulazi))
	imena := make([]string, 0, len(r.Ulazi))
	for _, u := range r.Ulazi {
		l, v, err := RastaviLetvu(u)
		if err != nil {
			fmt.Fprintln(w, err)
			return nil
		}
		izvori = append(izvori, Izvor{Letva: l, Velicina: v})
		imena = append(imena, l+" ("+jedinicaVelicine(v)+")")
	}
	zaglavlje := letva + " ← " + strings.Join(imena, " + ")

	pojasi, err := NamjestiLetvu(arhiva, letva, vel, izvori)
	if err != nil {
		fmt.Fprintf(w, "%-56s %v\n", zaglavlje, err)
		return nil
	}
	fmt.Fprintf(w, "%-56s %s\n", zaglavlje, vel)
	ispisiPojase(w, pojasi, letva)
	...
	for i, rez := range Rezerve[letva] {
		...
		if err != nil {
			fmt.Fprintf(w, "   rezerva %d (%s): %v\n", i+1, strings.Join(rez, " + "), err)
			continue
		}
		fmt.Fprintf(w, "   rezerva %d:\n", i+1)
		ispisiPojase(w, rezervni, letva)
...
func namjestiInacicu(arhiva *sql.DB, letva, vel string, ulazi []string, inacica int) ([]Pojas, error) {
	izvori := make([]Izvor, 0, len(ulazi))
	for _, u := range ulazi {
		l, v, err := RastaviLetvu(u)
		if err != nil {
			return nil, err
...
func ispisiPojase(w io.Writer, pojasi []Pojas, letva string) {
	...
	for _, p := range pojasi {
		...
		fmt.Fprintf(w, "   %8.0f – %8.0f %-4s  r %.3f   rasap %7.1f %-4s  (%d sati)%s\n",
			p.Od, p.Do, jedinicaVelicine(p.Ulazi[0].Velicina), p.R, p.Rasap, jedinicaVelicine(p.Velicina), p.Sati, oznaka)
		for _, u := range p.Ulazi {
			fmt.Fprintf(w, "        %-16s -%2d h   prozor %2d h   nagib %6.2f %s\n",
				u.Letva, u.PomakH, u.Sirina, u.Nagib, poJedinici(p.Velicina, u.Velicina))
		}
	}
}

func jedinicaVelicine(velicina string) string {
	if velicina == "protok" {
		return "m³/s"
	}
	return "cm"
...
func poJedinici(cilj, ulaz string) string {
	...
	return jedinicaVelicine(cilj) + "/" + jedinicaVelicine(ulaz)
}

// namjestanjeTraje čuva da se lanac ne namješta dvaput odjednom: rijetke
// letve na trenutak spuštaju granice namještanja, a to su zajedničke
// postavke paketa.
var namjestanjeTraje sync.Mutex

// OpcijeNamjestanja su postavke jednog namještanja lanca.
type OpcijeNamjestanja struct {
	Probno  bool      // samo ispiši što bi se namjestilo
	Dnevnik io.Writer // ispis tijeka; nil znači nikamo
	Korak   func(sto string, gotovo, ukupno int)
}

// NamjestiLanac namjesti sve račune lanca, spremi pojase i modele ispuštanja
// elektrana u bazu prognoza i vrati broj spremljenih pojasa. Arhiva se samo
// čita. Granice NamjestiDo i NamjestiBez vrijede kako su postavljene.
func NamjestiLanac(arhiva, baza *sql.DB, o OpcijeNamjestanja) (int, error) {
	if !namjestanjeTraje.TryLock() {
		return 0, fmt.Errorf("namještanje lanca već traje")
	}
	defer namjestanjeTraje.Unlock()
	w := o.Dnevnik
	if w == nil {
		w = io.Discard
	}
	...
	for _, tok := range Tokovi {
		...
		for _, r := range tok.Racuni {
			if o.Korak != nil {
				o.Korak("namještam "+r.Letva, gotovo, ukupno)
			}
			sve = append(sve, namjestiRacun(arhiva, r, w)...)
			gotovo++
		}
	}
	if o.Probno {
	...
	if len(sve) == 0 {
		return 0, fmt.Errorf("nijedan račun lanca se nije dao namjestiti — ništa nije spremljeno")
	}
	if err := Spremi(baza, sve, time.Now().Format(time.RFC3339)); err != nil {
		return 0, err
	}
	fmt.Fprintf(w, "\nspremljeno %d pojasa\n", len(sve))

	// Modeli ispuštanja elektrana: uče se na svemu do granice (NamjestiDo ili
	...
	for _, op := range Operateri {
		if o.Korak != nil {
			o.Korak("model ispuštanja "+op.Letva, gotovo, ukupno)
		}
		m, err := NamjestiOperatera(arhiva, op, ocjenaOd)
		if err != nil {
			fmt.Fprintf(w, "model ispuštanja %s: %v\n", op.Letva, err)
			continue
		}
		...
		for g, ime := range []string{"obična voda", "velika voda"} {
			fmt.Fprintf(w, "  %-12s", ime)
			for _, k := range []int{3, 6, 12, 18, 24, 36, 48, 72, 96} {
				z := " "
				if m.Koristi[g][k] {
					z = "*"
				}
				fmt.Fprintf(w, "  %2dh %5.1f/%5.1f%s", k, m.MAE[g][k], m.MAEPost[g][k], z)
			}
			fmt.Fprintln(w)
		}
	}
...
// ProbajRacun namjesti jednu postavku, npr. "aljmas = batina + belisce", i
// ispiše je; ništa ne sprema.
func ProbajRacun(arhiva *sql.DB, zapis string, w io.Writer) error {
	if !namjestanjeTraje.TryLock() {
		return fmt.Errorf("namještanje lanca već traje")
	}
	defer namjestanjeTraje.Unlock()
	r, err := RazaberiRacun(zapis)
	if err != nil {
		return err
	}
	namjestiRacun(arhiva, r, w)
	return nil
}
